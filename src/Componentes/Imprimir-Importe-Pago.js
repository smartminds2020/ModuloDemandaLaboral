...
class ImprimirImportePago extends React.Component {
  ...
  Imprimir(){
    ...
    var checkbox_selec=[];
    var checks=[];
    var nombres = this.props.alumno.apeNom;//detalles de datos del alumno

    var codigo= this.props.alumno.codigo;
    var importe = 0;
    var listadopagos = this.props.listado;//lista de pagos
    var listado = [];
    var listafinal = [];
    var total=[];
    ...
    var headCostoReal = null;
    var bodyCostoReal = null;
    var dataCostoReal = null;
    var headCostoFinal = null;
    ...
    if(this.props.costos.tipo == "por credito") // // NOTE: estas weas son las que cambian segun el credito o ciclo :v
    {
      ...
      headCostoReal = ["Matrícula UPG","Matrícula EPG","Derecho Enseñanza","Total","Valor por Crédito"];
      bodyCostoReal = ["             ","             ","                 ","     ","                 "];
      dataCostoReal = [[this.props.costos.upg,this.props.costos.epg,this.props.costos.total,this.props.costos._Total,this.props.costos.creditos+ " x "+this.props.costos.costo_credito ]]
    
      headCostoFinal = ["Matrícula UPG","Matrícula EPG","Derecho Enseñanza","Total","Valor por Crédito","Dscto. Aplicado"];
      bodyCostoFinal = ["             ","             ","                 ","     ","                 ","               "];
      dataCostoFinal = [[this.props.costos.d_upg,this.props.costos.d_epg,this.props.costos.d_total,this.props.costos.d_Total,this.props.costos.creditos+ " x "+this.props.costos.costo_credito_d,this.props.costos.tipo ] ] 
         ...
         headCostoReal = ["Matrícula UPG","Matrícula EPG","Derecho Enseñanza","Total","Valor por Ciclo"];
         bodyCostoReal = ["             ","             ","                 ","     ","                 "];
         dataCostoReal = [[this.props.costos.upg,this.props.costos.epg,this.props.costos.total,this.props.costos._Total,this.props.costos.creditos+ " x "+this.props.costos.costo_credito ]  ]
       
         headCostoFinal = ["Matrícula UPG","Matrícula EPG","Derecho Enseñanza","Total","Valor por Ciclo","Dscto. Aplicado"];
         bodyCostoFinal = ["             ","             ","                 ","     ","                 ","               "];
         dataCostoFinal = [[this.props.costos.d_upg,this.props.costos.d_epg,this.props.costos.d_total,this.props.costos.d_Total,this.props.costos.creditos+ " x "+this.props.costos.costo_credito_d,this.props.costos.tipo ]  ]
       }
...
/*
    ...
    console.log(conceptos);
      */
      ...
      //CONCEPTO DE LOS CONCEPTOS SELECCIONADOS
    var conceptos=[];
    var aux = listafinal;
  ...
  for (let l = 0; l<listafinal.length; l++) {
    ...
    //THIS.PROPS.SELECCIONADO NO EXISTE
    //SE ELIMINA EL IF / ELSE DE SELECCIONADO
    //CAMBIAR EL FORMATO
    var arrayAuxiliar=[];
    ...
    for (let m = 0; m<arrayAntes.length; m++) {
     ...
     var pago = [m+1,arrayAntes[m].ciclo.toString(),arrayAntes[m].concepto,arrayAntes[m].numero,arrayAntes[m].fecha.replace(/^(\d{4})-(\d{2})-(\d{2})$/g,'$3-$2-$1')
      ...
      var agregarTotal = [ ,,,,"Total","S/."+this.comita(totalizado.toString()),]
    ...
    if(listafinal[l][0].concepto=="210010  "){
  ...
  console.log("select"+ this.props.seleccionado);
    ...
    //if(this.props.seleccionado){
    var columns = ["N°","Ciclo","Concepto","N° Recibo","Fecha","Importe","Revisado"];
     var columns2 = ["  ","     ","        ","         ","     ","       ","        "];
     var columnsBenf2 = ["  ","         ","            ","         ","     ","             "];
  var columnaPrueba = [["  ","     ","        ","         ","     ","       ","        "]];
     
    var columnsCosto = ["  ","     ","        ","         ","Costo","S/."+this.comita(this.props.valorCosto1.toString()),"        "];
...
//******************* */
    ...
    doc.setFontSize(10);
       ...
       doc.text("COSTO REAL ",38, 230);
   

   doc.autoTable(headCostoReal, dataCostoReal, {
     theme: 'grid',
     styles: {
         cellPadding: 5, // a number, array or object (see margin below)
         fontSize: 8,
 ...
 var first = doc.autoTable.previous;
    ...
    doc.setFontSize(10);
       ...
       doc.text("COSTO FINAL ",38, first.finalY + 25);
...
//DATOS DE LOS CONCEPTOS
          ...
          var first = doc.autoTable.previous;
           ...
           doc.setFontType("bold");
           doc.setFontSize(10);
           if(listadoFinalFormato[0][0][2] == "210010  "){
            doc.text("MATRÍCULA UPG ",38, first.finalY + 25);
          ...
          doc.autoTable(columnsCosto, columnaPrueba, {
            theme: 'grid',
            styles: {
                cellPadding: 5, // a number, array or object (see margin below)
                fontSize: 8,
        ...
        //Mostramos el encabezado de la primera tabla
            ...
            for (let k = 1; k<listadoFinalFormato.length; k++) {
              ...
              doc.setFontType("bold");
              doc.setFontSize(10);
              if(listadoFinalFormato[k][0][2] == "210010  "){
                doc.text("MATRÍCULA UPG ",38, first.finalY + 25);
                columnsCosto = ["  ","     ","        ","         ","Costo","S/."+this.comita(this.props.valorCosto1.toString()),"        "];

              }else if(listadoFinalFormato[k][0][2] == "207010  "){
                doc.text("MATRÍCULA EPG ",38, first.finalY + 25);
