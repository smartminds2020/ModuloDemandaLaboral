...
class formulario extends React.Component{
    ...
    guardar(){
        ...
        var fechaInicial=document.getElementById("fecha").value.replace(/^(\d{2})[-\/](\d{2})[-\/](\d{4})$/g,'$3-$2-$1');
        var fecha=new Date(fechaInicial+"T00:00:00");
       ...
       &&this.state.valorTipo!=""){
        ...
        if(parseInt(valor)<=parseInt(this.state.maximo)){
            ...
            if(this.state.abp >= 0){
               ...
               fetch(CONFIG+"beneficio/insertar",
                {
                headers: {
