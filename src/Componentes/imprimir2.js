...
class Imprimir2 extends React.Component {
  ...
  Imprimir(){
    ...
    {           
                 ...
                 var costoReal = [costoRealLabel, "S/. "+this.comita(costoUPG.toString()),"S/. "+this.comita(costoEPG.toString()),"S/. "+this.comita(costoTotal.toString()),"S/. "+this.comita(costo_TOTAL.toString()),`${creditos} x ${costo_credito} por credito`];
                 var costoFinal = [costoFinalLabel,"S/. "+this.comita(dcostoUPG.toString()),"S/. "+this.comita(dcostoEPG.toString()),"S/. "+this.comita(dcostoTotal.toString()),"S/. "+this.comita(dcosto_TOTAL.toString()),`${creditos} x ${costo_credito_d} por credito`]
               
                 if(this.props.datos.length>0)                 
                  costoDelPrograma.push(costoReal,costoFinal);                 
                 else                 
                   costoDelPrograma.push(costoReal);
                 ...
                 var headerPorCredito = [' ','Matricula UPG','Matricula EPG','Derecho Enseñanza','Total','Costo Aplicado']
                 var costoReal = [costoRealLabel, "S/. "+this.comita(costoUPG.toString()),"S/. "+this.comita(costoEPG.toString()),"S/. "+this.comita(costoTotal.toString()),"S/. "+this.comita(costo_TOTAL.toString()),`${creditos} x ${costo_credito} por ciclo`];
                 var costoFinal = [costoFinalLabel,"S/. "+this.comita(dcostoUPG.toString()),"S/. "+this.comita(dcostoEPG.toString()),"S/. "+this.comita(dcostoTotal.toString()),"S/. "+this.comita(dcosto_TOTAL.toString()),`${creditos} x ${costo_credito_d} por ciclo`]
...
/**************************************************************** */
    ...
    doc.setFontType("normal");
    doc.setFontSize(9);
    doc.text(listafinal[0][0].dni, 610, 160);

     //TERCER CUADRO DE INFORMACION

     doc.setDrawColor(0, 0, 0);
     doc.setLineWidth(0.5);
     doc.line(35,195,750,195);
     /*
     doc.setDrawColor(0, 0, 0);
     doc.setLineWidth(0.5);
    ...
    doc.addImage(imgData, 'JPG', 35, 12, 50, 60);
      ...
      //    console.log(listadoFinalBeneficio);
          ...
          var first = doc.autoTable.previous;
           ...
           //Mostramos el encabezado de la primera tabla
           let descConcepto=(this.props.seleccionado) ? listadoFinalFormato[0][0][10] : listadoFinalFormato[0][0][9];
           doc.setFont("helvetica");
           doc.setFontType("bold");
           doc.setFontSize(10);
           //doc.text("PAGO POR CONCEPTO "+ descConcepto,38,first.finalY + 25);
           doc.text("PAGO POR CONCEPTO",38,first.finalY + 25);           
           console.log("listadofinalFormato ",listadoFinalFormato)
            ...
            for (let k = 1; k<listadoFinalFormato.length; k++) {
              var first = doc.autoTable.previous;
              let encabezado=(this.props.seleccionado) ? listadoFinalFormato[k][0][10] : listadoFinalFormato[k][0][9];
